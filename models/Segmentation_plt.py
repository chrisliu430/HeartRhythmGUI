...
def std_range(data_test):
    ...
    return data_all_feature1

def train_S2_Kmeans(folder, kmeans_model_S1):
    file_names = glob.glob(os.path.join(folder, '*.wav'))
    data = np.array([])
    for file_name in file_names:
        try:
            down_sample_data, No_S1_signal = S1_segmentation(file_name, kmeans_model_S1)
            print(file_name)
            down_sample_data = remove_S1(down_sample_data, No_S1_signal)
            cut_data = feature_extraction_origin(down_sample_data, 1000)
            std_range_data = std_range(cut_data)
            data = np.append(data, std_range_data)
        except:
            print('This file is fail')

    data = np.reshape(data, (-1,2))
    kmeans_fit = cluster.KMeans(n_clusters=2).fit(data)
    pickle.dump(kmeans_fit, open("Kmeans_model_S2_phosioNet.h5", "wb"))
...
def S2_segmentation(x, No_S1_point, model):
    ...
    original_data = x
    S2_center = center_number_new.flatten()
    S2_center = S2_center[1:-1].reshape(-1,2)
    S1_center = No_S1_point.flatten()
    S1_center = S1_center[1:-1].reshape(-1, 2)
    Sys_data = np.array([])
    dis_data = np.array([])
    if(S2_center.shape[0] > S1_center.shape[0]):
        max_data_number = S1_center.shape[0]
    else:
        max_data_number = S2_center.shape[0]
    for i in range(max_data_number-2):
        Sys_data = np.append(Sys_data, original_data[int(S1_center[i, 1]) : int(S2_center[i, 0])])
        dis_data = np.append(dis_data, original_data[int(S2_center[i, 1]) : int(S1_center[i+1, 0])])
...
############################################################
#plt figure
def plt_S1(x, center_number_new):
    # 讀取音頻
    audio_data, fs = librosa.load(x, sr=2000, duration=10)
    # wavelet denoise
    audio_data1 = wavelet_denoise.paper_wavelet(audio_data)
    ...
    range_x = np.array(range(int(center_number_new[0][0]-100), int(center_number_new[0][0])))
    ...
    #range_x = np.array(range(int(center_number_new[len(center_number_new) - 1][1]), 10000))
    axs[1].fill_between(range_x, -1, 1, facecolor='green', alpha=0.3)
    return plt.show()
...
####################################################################################################
    ...
    #axs[1].fill_between(range_x, -1, 1, facecolor='green', alpha=0.3)
######################################################################################################
    range_y = np.array(range(int(center_number_S2[0][0]-100), int(center_number_S2[0][0])))
    ...
    for i in range(0, len(center_number_S2) - 1):  # len(max_value_x1)):
        ...
        axs[1].fill_between(range_y, -1, 1, facecolor='red', alpha=0.3)
    #range_y = np.array(range(int(center_number_S2[len(center_number_S2) - 1][1]), int(center_number_S2[len(center_number_S2) - 1][1]+10000)/2))
    #axs[1].fill_between(range_y, -1, 1, facecolor='red', alpha=0.3)
#######################################################################################################
