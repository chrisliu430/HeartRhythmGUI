...
def S2_segmentation(x, No_S1_point, model):
    ...
    # 擷取資料的統計量
    x_1 = down_sample_audio_data1
    original_data = copy.deepcopy(down_sample_audio_data1)
    down_sample_rate = 1000
    ...
    for i in range(max_data_number-2):
        Sys_data = np.append(Sys_data, original_data[int(S1_center[i+1, 1]) : int(S2_center[i+1, 0])])
        dis_data = np.append(dis_data, original_data[int(S2_center[i+1, 1]) : int(S1_center[i+2, 0])])
    #print('1=',Sys_data)
    #print('2=',dis_data)
    return Sys_data, dis_data
...
def extract_feature(file_name, model, model_S2):
    ...
    Sys_data, dis_data = S2_segmentation(file_name, no_s1_center, model_S2)
    dis_data = np.array(dis_data)
    resize_dis_data = np.resize(dis_data, (10000,))
    dis_mfccs = np.mean(librosa.feature.mfcc(y=resize_dis_data, sr=1000, n_mfcc=20).T, axis=0)
    # S2_mfccs = S2_mfccs / np.max(np.abs(S2_mfccs))
    dis_feature = np.array(dis_mfccs).reshape([-1, 1])
    return dis_feature
...
if __name__ == '__main__':
    ...
    model = load_model('./models/CNN_20_ASD_normal.hdf5')
